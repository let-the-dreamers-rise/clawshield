...
const TIER_LABELS: Record<VerifiedTier, string> = {
  none: "Unverified",
...
const tierStyles: Record<VerifiedTier, string> = {
  ...
  enterprise: "bg-cyan/10 text-cyan border-cyan/30",
};

export interface ClawShieldVerifiedBadgeProps {
  tier: VerifiedTier;
  size?: "sm" | "md" | "lg";
  showLabel?: boolean;
  className?: string;
...
export function ClawShieldVerifiedBadge({
  tier,
  size = "md",
  showLabel = true,
  className,
}: ClawShieldVerifiedBadgeProps) {
  if (tier === "none") {
    return showLabel ? (
      <span className={cn("text-xs text-text-dim", className)}>Unverified</span>
    ) : null;
  }
